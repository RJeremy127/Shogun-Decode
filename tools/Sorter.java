...
public class Sorter {
    ...
    public static class SpindexerPositions {
        ...
        public static double fullRotation = 0.6653;
        public static double shootTolerance = 0.05;
    }

    public static class SpindexerDelays {
        public static double transferDurationMs = 2000;
        public static double settleTimeMs = 200;
        public static double moveWaitMs = 300;
        public static double intakeServoSpeed = 0.8;
        public static double sortingServoSpeed = 0.8;
        public static double shootHighVelSpeed = 0.3;
        public static double shootLowVelSpeed = 0.3;
        public static double shootVelocityThreshold = 1300;
    ...
    private static int transferStep = 0;
    private static double transferStepStartPos = 0;
    private static double shotWaitStartTime = 0;
    private static boolean waitingAfterShot = false;

    private static boolean sortingMode = false;
    ...
    public static void preloadBalls(String... colors) {
        ports = new String[3];
    ...
    public static void updateSlew() {
        ...
        slewTimer.reset();
        double speed = getActiveServoSpeed();
    ...
    public static void advanceSpindexer() {
        if (currentPort < 2) {
            ...
            // At port 2 (3rd ball) — nudge servo forward so ball physically clears into slot
            double nudge = SpindexerPositions.slot2 - SpindexerPositions.slot1;
            setPosition(Math.min(currentPos + nudge, 1.0));
        }
    }
    ...
    private static void beginTransferSpin() {
        transferStepStartPos = currentPos;
        originalBallCount = getBallCount();
        transferActive = true;
        transferPending = false;
        transferStartTime = timer.milliseconds();
        transferStep = 0;
        waitingAfterShot = false;
        if (SpindexerDelays.shootDelayMs > 0) {
            // Step-based: first ball is already at shooter, start wait immediately
            shotWaitStartTime = timer.milliseconds();
            waitingAfterShot = true;
        } else {
            // Legacy: single full-rotation
            setPosition(transferStepStartPos + SpindexerPositions.fullRotation);
        }
    }
    ...
    public static int handleTransfer() {
        ...
        if (SpindexerDelays.shootDelayMs > 0) {
            // Step-based: wait after each ball fires, then advance to next
            if (waitingAfterShot) {
                if (timer.milliseconds() - shotWaitStartTime >= SpindexerDelays.shootDelayMs) {
                    waitingAfterShot = false;
                    transferStep++;
                    if (transferStep >= 3) {
                        transferActive = false;
                    ...
                    setPosition(transferStepStartPos + transferStep * (SpindexerPositions.fullRotation / 3.0));
                }
            } else {
                if (isSettled()) {
                    shotWaitStartTime = timer.milliseconds();
        ...
        // Legacy: time-based full rotation
    ...
    public static void setSortingMode(boolean sorting) {
        sortingMode = sorting;
    }
    ...
    public static boolean isJammed() {
        ...
        double elapsed = timer.milliseconds() - transferStartTime;
        return elapsed > TransferSettings.transferTimeoutMs * 0.8
                && Math.abs(currentPos - targetPos) > 0.05;
    }

    public static double getActiveServoSpeed() {
        if (transferActive) {
            double vel = Flywheel.getVelocity();
            return vel >= SpindexerDelays.shootVelocityThreshold
                    ? SpindexerDelays.shootHighVelSpeed
                    : SpindexerDelays.shootLowVelSpeed;
        }
        return sortingMode ? SpindexerDelays.sortingServoSpeed : SpindexerDelays.intakeServoSpeed;
    }
